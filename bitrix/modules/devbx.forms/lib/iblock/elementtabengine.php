...
class ElementTabEngine
{
    ...
    public static function saveData($iblockElementInfo)
    {
        return true;
    }
    ...
    public static function getTabs($iblockElementInfo)
    {
        ...
        if ($iblockElementInfo["ID"] > 0 && (!isset($request['action']) || $request['action'] != 'copy')) {
            ...
            while ($arForm = $dbForm->fetch()) {
                ...
                if (is_array($arForm['SETTINGS']['FIELD_IBLOCK_ELEMENT_TAB'])) {
                    foreach ($arForm['SETTINGS']['FIELD_IBLOCK_ELEMENT_TAB'] as $field => $value) {
                        if ($value == 'Y')
                        {
                ...
                if (!empty($formUserFields))
                {
                    ...
                    foreach ($formUserFields as $fieldName)
                    {
                        ...
                        if ($arUserField['SETTINGS']['IBLOCK_ID'] == $iblockElementInfo['IBLOCK']['ID'])
                        {
                            $addTab = true;
                        } elseif (empty($arUserField['SETTINGS']['IBLOCK_ID']))
                        {
                            $arResult = $obForm->getDataClass()::getList([
                                'filter' => [
                                    '='.$fieldName => $iblockElementInfo['ID']
                                ],
                                'select' => [
                                    'ID'
                                ],
                        ...
                        if ($addTab)
                        {
                            ...
                            $arTabs[] = array(
                                ...
                                "SORT" => 4,
                                "TAB" => Loc::getMessage('DEVBX_FORMS_IBLOCK_ELEMENT_TAB', $arReplace),
                                "TITLE" => Loc::getMessage('DEVBX_FORMS_IBLOCK_ELEMENT_TAB_TITLE', $arReplace),
                            );
                        }
                    }
                }
    ...
    public static function showTab($div, $iblockElementInfo)
    {
        ...
        <tr>
            <td colspan="2">
                ...
                $ELEMENT_ID = $iblockElementInfo['ID'];
