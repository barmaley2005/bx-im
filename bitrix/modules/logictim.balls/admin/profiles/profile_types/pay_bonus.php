<?
$APPLICATION->SetTitle(GetMessage("logictim.balls_BONUS_PAY_ORDER"));

if($request['id'] == 'new')
{
	$jsonProductConditions = \Logictim\Balls\Conditions\PayBonus::BaseConditions('json');
...
$aTabs = array(
	...
	array("DIV" => "logictim_balls_tab_3", "TAB" => GetMessage("logictim.balls_PRODUCTS_COND_PAY_TAB"), "TITLE" => GetMessage("logictim.balls_PRODUCTS_COND_PAY_TAB"))
);
...
if((!empty($request['apply']) || !empty($request['save'])) && check_bitrix_sessid())
{
	if(!empty($request['id']))
	{
		...
		$otherConditions = array(
									"MIN_PAYMENT_BONUS" => $request['min_payment_bonus'],
									"MIN_PAYMENT_TYPE" => $request['min_payment_type'],
									"MIN_PAYMENT_INCLUDE_SHIPPING" => $request['min_payment_include_shipping'],
									"MAX_PAYMENT_BONUS" => $request['max_payment_bonus'],
									"MAX_PAYMENT_TYPE" => $request['max_payment_type'],
									"MAX_PAYMENT_INCLUDE_SHIPPING" => $request['max_payment_include_shipping'],
									"PAYMENT_ROUND" => $request['payment_round'],
								);
...
<section class="">
    ...
    <form class="logictim_profile" name="logictim_profile" method="post" action="<?echo $APPLICATION->GetCurPage()?>?lang=<?=LANGUAGE_ID?>">
		...
		<? $tabControl->BeginNextTab();?>
				...
				{
					$active = 'Y';
					$profileName = GetMessage("logictim.balls_BONUS_PAY_ORDER");
					$sort = 100;
				...
				<tr><td width="40%"><?=GetMessage("logictim.balls_PROFILE_TYPE")?></td><td><?=GetMessage("logictim.balls_BONUS_PAY_ORDER")?></td></tr>
                <? if($profileId > 0) {?>
            ...
            <tr>
            	<td width="40%"><?=GetMessage("logictim.balls_BONUS_PAY_MIN")?></td>
                <td>
                	<input type="text" size="3" name="min_payment_bonus" value="<?=$min_payment?>">
                    
                    <select name="min_payment_type" style="margin-left:5px;">
                    	<option value="bonus" <? if($min_payment_type == 'bonus') echo 'selected="selected"'?>><?=GetMessage("logictim.balls_BONUS_BONUS")?></option>
                        <option value="percent" <? if($min_payment_type == 'percent') echo 'selected="selected"'?>><?=GetMessage("logictim.balls_BONUS_PERCENT")?></option>
                    </select>
                    <?=GetMessage("logictim.balls_COND_PART_ORDER")?>
                    <select name="min_payment_include_shipping" style="margin-left:5px;">
                    	<option value="Y" <? if($min_payment_include_shipping == 'Y') echo 'selected="selected"'?>><?=GetMessage("logictim.balls_BONUS_INCLUDE")?></option>
                        <option value="N" <? if($min_payment_include_shipping == 'N') echo 'selected="selected"'?>><?=GetMessage("logictim.balls_BONUS_NO_INCLUDE")?></option>
                    </select>
                    <?=GetMessage("logictim.balls_COND_PART_INCLUDE_DELIVERY")?>
                </td>
            </tr>
            <tr>
            	<td width="40%"><?=GetMessage("logictim.balls_BONUS_PAY_MAX")?></td>
                <td>
                	<input type="text" size="3" name="max_payment_bonus" value="<?=$max_payment?>">
                    
                    <select name="max_payment_type" style="margin-left:5px;">
                    	<option value="bonus" <? if($max_payment_type == 'bonus') echo 'selected="selected"'?>><?=GetMessage("logictim.balls_BONUS_BONUS")?></option>
                        <option value="percent" <? if($max_payment_type == 'percent') echo 'selected="selected"'?>><?=GetMessage("logictim.balls_BONUS_PERCENT")?></option>
                    </select>
                   <?=GetMessage("logictim.balls_COND_PART_ORDER")?>
                    <select name="max_payment_include_shipping" style="margin-left:5px;">
                    	<option value="Y" <? if($max_payment_include_shipping == 'Y') echo 'selected="selected"'?>><?=GetMessage("logictim.balls_BONUS_INCLUDE")?></option>
                        <option value="N" <? if($max_payment_include_shipping == 'N') echo 'selected="selected"'?>><?=GetMessage("logictim.balls_BONUS_NO_INCLUDE")?></option>
                    </select>
                    <?=GetMessage("logictim.balls_COND_PART_INCLUDE_DELIVERY")?>
                </td>
            </tr>
            <tr>
            	<td width="40%"><?=GetMessage("logictim.balls_BONUS_PAY_ROUND")?></td>
                <td>
               ...
               <?=GetMessage("logictim.balls_BONUS_PAY_ROUND_1")?>
               </td>
            </tr>
           ...
           <tr class="heading" id="tr_BT_SALE_DISCOUNT_SECT_APP"><td colspan="2"><?=GetMessage("logictim.balls_PROFILE_COND_SECT")?></td></tr>
            ...
            <tr class="heading" id="tr_BT_SALE_DISCOUNT_SECT_APP"><td colspan="2"><?=GetMessage("logictim.balls_PRODUCTS_COND_PAY_SECT")?></td></tr>
            <tr><td width="100%" colspan="2">
            	<div id="ProductsConditions"></div>
                <script>
                    var JSSaleAct=new BX.TreeConditions(<?=\Logictim\Balls\Conditions\PayBonus::MainParams('json');?>,<?=$jsonProductConditions?>,<?=\Logictim\Balls\Conditions\PayBonus::Controls('json')?>);
                </script>
            </td></tr>
