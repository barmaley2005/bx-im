...
class BonusFromOrderAdd
{
	public static function BonusAdd($order, $params = array())
	{
		
		//Proveryaem dostupnost' modulya dlya sayta
		$site_id = $order->getSiteId();
		if(cHelper::CheckSiteDostup($site_id) != 'Y')
			return;
		
		
		cBonusFromReferal::BonusAdd($order, $params);
		...
		$order_cart_sum = $order_price - $order_delivery_sum;
			...
			$UserBonusSystemDostup = cHelper::UserBonusSystemDostup($order_user_id);
			$UserBallance = cHelper::UserBallance($order_user_id);
			
			if(CModule::IncludeModule("sale") && CModule::IncludeModule("iblock") && $UserBonusSystemDostup == 'Y')//Esli pereveli zakaz v status "Vipolnen"
			{
				...
				if(empty($OperationAddFields)) //Esli bonusi po zakazu eshe ne bili nachisleni, to schitaem i nachislyaem
				{
					...
					$allBonus = $arBonus["ALL_BONUS"];
					$detailBonusProd = $arBonus["COMMENT_FOR_OPERATION"];
					...
					if($allBonus > 0 && $order_user_id > 0)
					{
						...
						if((int)COption::GetOptionString("logictim.balls", "BONUS_ORDER_WAIT", 0) > 0):
							...
							$PROP = array();
							$PROP["OPERATION_TYPE"] = Array("VALUE" => $operationsTypeWait['ADD_FROM_ORDER']);
							$PROP["USER"] = $order_user_id;
							...
							$newOperationArray = Array(
													...
													"NAME"           => GetMessage("logictim.balls_BONUS_ADD_USER").$order_num,
													"ACTIVE"         => "Y",
													"CODE" => 'API_OPERATIONS'
													);
							...
							$addBonusProp->setValue($allBonus);
							$addBonusProp->save();
							...
							$newOperation = new CIBlockElement;
							$PROP = array();
							$PROP["OPERATION_TYPE"] = Array("VALUE" => $operationsType['ADD_FROM_ORDER']);
							$PROP["USER"] = $order_user_id;
							$PROP["OPERATION_SUM"] = $allBonus;
							...
							if(COption::GetOptionString("logictim.balls", "BONUS_BILL", '1') == 1)
							{
								...
								$USER_FIELD_MANAGER->Update("USER", $order_user_id, array("UF_LOGICTIM_BONUS" => $updateUserBonus));
							}
							//Esli ispol'zuetsya vnutrenniy schet bitrix
							else
							{
								$currency = COption::GetOptionString("logictim.balls", "BONUS_CURRENCY", 'RUB');
								CSaleUserAccount::UpdateAccount($order_user_id, +$allBonus, $currency, GetMessage("logictim.balls_BONUS_ADD_USER").$order_num, $order_id);
							}
							
							//Otmechaem v zakaze, chto bonusi nachisleni
							//CSaleOrderPropsValue::Update($order_props["LOGICTIM_ADD_BONUS"]["ID"], array("VALUE" => $allBonus)); //OLD MHETOD
							$addBonusProp = $db_order_props->getItemByOrderPropertyId($order_props["LOGICTIM_ADD_BONUS"]["ORDER_PROPS_ID"]);
							$addBonusProp->setValue($allBonus);
							$addBonusProp->save();
							...
							//Otpravlyaem email s uvedomleniem
								//Sozdaem pochtovoe sobitie bitrix
								...
								if($liveBonus == 'Y')
									...
									$bonusLiveDate = '-';
								CEvent::Send("LOGICTIM_BONUS_FROM_ORDER_ADD", cHelper::SitesId(), 
									array( 
										"ORDER_ID" => $order_id, 
										"BONUS" => $allBonus,
