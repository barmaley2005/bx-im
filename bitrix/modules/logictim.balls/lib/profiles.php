...
class Profiles {
								...
								)
	{
		...
		/* Esli proishodit nachislenie za zakaz, to dlya opredeleniya aktivnosti pravila berem datu zakaza */
		if($arParams['PROFILE_TYPE'] == 'order' && isset($arParams['ORDER']) && isset($arParams['ORDER']['ORDER_ID']) && (int)$arParams['ORDER']['ORDER_ID'] > 0)
		{
			$order = \Bitrix\Sale\Order::load($arParams['ORDER']['ORDER_ID']);
			$orderDateInsertObj = $order->getDateInsert();
			$nowTime = $orderDateInsertObj->getTimestamp();
		}
		/* Esli proishodit nachislenie za zakaz, to dlya opredeleniya aktivnosti pravila berem datu zakaza */
		...
		{
			//check active_from
			if($arProfile['active_from'] == '0000-00-00 00:00:00')
				$arProfile['active_from'] = '';
			if($arProfile['active_to'] == '0000-00-00 00:00:00')
				$arProfile['active_to'] = '';
