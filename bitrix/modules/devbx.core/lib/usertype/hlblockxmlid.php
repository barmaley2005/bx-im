...
class HLBlockXmlId extends BaseType
{
    const USER_TYPE_ID = "devbx_hlblock_xmlid";
    ...
    public static function GetUserTypeDescription()
    {
        return array(
            ...
            "DESCRIPTION" => Loc::getMessage('DEVBX_USERTYPE_HLBLOCK_XML_ID_DESCRIPTION'),
            "BASE_TYPE" => 'string',
        );
    }
    ...
    public static function PrepareSettings($arUserField)
    {
        ...
        $element_id = $arUserField["SETTINGS"]["DEFAULT_VALUE"];
    ...
    public static function GetSettingsHTML($arUserField, $arHtmlControl, $bVarsFromForm)
    {
        ...
        if(Loader::includeModule('highloadblock'))
        {
            ...
            $result .= '
			<tr>
				<td>'.Loc::getMessage("DEVBX_USERTYPE_HLBLOCK_XML_ID_DISPLAY").':</td>
				<td>
					'.$dropDown.'
				</td>
			</tr>
        ...
        if($hlblock_id > 0 && mb_strlen($hlfield_id) && Loader::includeModule('highloadblock'))
        {
            $result .= '
			<tr>
				<td>'.Loc::getMessage("DEVBX_USERTYPE_HLBLOCK_XML_ID_DEFAULT_VALUE").':</td>
				<td>
					<select name="'.$arHtmlControl["NAME"].'[DEFAULT_VALUE]" size="5">
						<option value="">'.Loc::getMessage("DEVBX_USERTYPE_HLBLOCK_XML_ID_NOT_SELECTED").'</option>
			';
            ...
            foreach ($rows as $row)
            {
                $result .= '<option value="'.$row["UF_XML_ID"].'" '.($row["UF_XML_ID"]==$value? "selected": "").'>'.htmlspecialcharsbx($row['VALUE']).'</option>';
            }
		...
		<tr>
			...
			<td>
				<label><input type="radio" name="'.$arHtmlControl["NAME"].'[DISPLAY]" value="'.self::DISPLAY_LIST.'" '.(self::DISPLAY_LIST==$value? 'checked="checked"': '').'>'.Loc::getMessage("DEVBX_USERTYPE_HLBLOCK_XML_ID_LIST").'</label><br>
				<label><input type="radio" name="'.$arHtmlControl["NAME"].'[DISPLAY]" value="'.self::DISPLAY_CHECKBOX.'" '.(self::DISPLAY_CHECKBOX==$value? 'checked="checked"': '').'>'.Loc::getMessage("DEVBX_USERTYPE_HLBLOCK_XML_ID_CHECKBOX").'</label><br>
			</td>
		</tr>
		';*/
        ...
        $result .= '
		<tr>
			<td>'.Loc::getMessage("DEVBX_USERTYPE_HLBLOCK_XML_ID_LIST_HEIGHT").':</td>
			<td>
    ...
    public static function getDropDownHtml($hlblockId = null, $hlfieldId = null)
    {
        ...
        $html .= '<option value="">'.htmlspecialcharsbx(Loc::getMessage('DEVBX_USERTYPE_HLBLOCK_XML_ID_SEL_HLBLOCK')).'</option>';
        ...
        $html .= '<option value="">'.htmlspecialcharsbx(Loc::getMessage('DEVBX_USERTYPE_HLBLOCK_XML_ID_SEL_HLBLOCK_FIELD')).'</option>';
        ...
        $html .= '
			<script type="text/javascript">
				function hlChangeFieldOnHlblockChanged(hlSelect)
				{
					...
					var newOption = new Option(\''.\CUtil::JSEscape(Loc::getMessage('DEVBX_USERTYPE_HLBLOCK_XML_ID_SEL_HLBLOCK_FIELD')).'\', "", false, false);
    ...
    {
        return static::GetAdminListEditHTML($arUserField, $arHtmlControl);
    }
    ...
    public static function GetAdminListEditHTML($arUserField, $arHtmlControl)
    {
        ...
        $iterator = static::GetList($arUserField);

        while ($ar = $iterator->Fetch())
        {
            $result['REFERENCE'][] = $ar['VALUE'];
            $result['REFERENCE_ID'][] = $ar['ID'];
        }

        return SelectBoxFromArray($arHtmlControl["NAME"], $result, $arHtmlControl["VALUE"], Loc::getMessage('DEVBX_USERTYPE_HLBLOCK_XML_ID_NOT_SELECTED'));
    }
