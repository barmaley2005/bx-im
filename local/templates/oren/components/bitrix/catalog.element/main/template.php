...
if ($arParams['QUICK_VIEW'] == 'Y')
{
    include($_SERVER['DOCUMENT_ROOT'].$templateFolder.'/quick_view.php');

} else {
    include($_SERVER['DOCUMENT_ROOT'].$templateFolder.'/full.php');
}

$obName = 'catalogElement'.$arResult['ID'];
...
?>
    <script>
        <?echo $obName?> = new DevBxCatalogElement(<?=\Bitrix\Main\Web\Json::encode($arJSParams)?>);
    </script>
<?
