...
namespace Local\Lib\Oren\Basket;
...
class Mutator {
    ...
    public static function apply(\CBitrixComponent $component, array &$result)
    {
        $component->arParams['BRAND_PROPERTY'] ??= '';

        $mobileColumns = $component->arParams['COLUMNS_LIST_MOBILE'] ?? $component->arParams['COLUMNS_LIST'];
        ...
        $property = $reflection->getProperty('basketItems');
        $property->setAccessible(true);
        $basketItems = $property->getValue($component);

        \Bitrix\Main\Loader::includeModule('logictim.balls');
        $arBonus = \cHelperCalc::CartBonus($basketItems);
        ...
        foreach ($basketItems as $row)
        {
            $rowData = array(
                ...
                'SHOW_LABEL' => false,
                'LABEL_VALUES' => array(),
                'BRAND' => isset($row[$component->arParams['BRAND_PROPERTY'].'_VALUE'])
                    ? $row[$component->arParams['BRAND_PROPERTY'].'_VALUE']
                    : '',
            );
            ...
            $rowData['BONUS_FORMATED'] = Loc::getMessage('OREN_BASKET_BONUS', array('#NUM#'=>$rowData['BONUS']));
            ...
            if (!empty($row['LABEL_ARRAY_VALUE']))
            {
                ...
                foreach ($row['LABEL_ARRAY_VALUE'] as $code => $value)
                {
                    $labels[] = array(
                        'NAME' => $value,
                        'HIDE_MOBILE' => !isset($component->arParams['LABEL_PROP_MOBILE'][$code])
                    );
                }
            ...
            {
                $rowData['PARENT_PRODUCT_ID'] = $arProductInfo['ID'];
            }
        ...
        if ($component->priceVatShowValue === 'Y')
        {
        ...
        if ($component->hideCoupon !== 'Y' && !empty($result['COUPON_LIST']))
        {
        ...
        $result['TOTAL_BONUS_FORMATED'] = Loc::getMessage('OREN_BASKET_TOTAL_BONUS', array('#NUM#'=>\CCurrencyLang::CurrencyFormat($result['TOTAL_BONUS'], 'RUB', false)));
    }
